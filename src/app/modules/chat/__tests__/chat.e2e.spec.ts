...
describe('Chat E2E Tests', () => {
  ...
  describe('Flow 4: Connection Removal with Chat Persistence', () => {
    ...
    it('non-participant cannot remove connection: 403', async () => {
      ...
      const { token: tokenC } = await createAuthUser(USER_ROLES.BROTHER, 'userC');
  ...
  describe('Flow 6 & 7: Chat List and Mark-Read Edge Cases', () => {
    ...
    async function setupTwoChats() {
      const { user: userA, token: tokenA } = await createAuthUser(USER_ROLES.BROTHER, 'userA');
      const { user: userB, token: tokenB } = await createAuthUser(USER_ROLES.BROTHER, 'Ali userB');
      const { user: userC, token: tokenC } = await createAuthUser(USER_ROLES.BROTHER, 'userC');
